...
// Verificador de senha SEPARADO: não persiste sessão — checar uma senha não troca a sessão do app
let _sbCheck=null;
async function verificarSenha(usuario, senha){
  try{
    if(!_sbCheck) _sbCheck=window.supabase.createClient(SUPA_URL, SUPA_KEY, {auth:{persistSession:false, autoRefreshToken:false, storageKey:'tg_pwcheck'}});
    const email=normNome(usuario)+'@togethere.app';
    const { error }=await _sbCheck.auth.signInWithPassword({ email, password:senha });
    if(error) return false;
    try{ await _sbCheck.auth.signOut({scope:'local'}); }catch(e){}
    return true;
...
const APP_VERSAO = '2026.08.07 · b181 (Supabase)';   // aparece no rodapé; mude a cada publicação no GitHub
...
async function cloudGet(timeout){
  ...
  try{
    ...
    const { data, error }=await sb.from('estado').select('dados,atualizado_em').eq('id','app').maybeSingle();
    if(error) return {ok:false};
    const st=(data && data.dados && Object.keys(data.dados).length)? data.dados : null;
    return {ok:true, state:st, rev:(data&&data.atualizado_em)||null};
  }catch(e){ return {ok:false}; }
}
function temAtividade(s){
  if(!s) return false;
  return ['presencas','material','temas','testes','contatos','planos','comentarios','atas','tarefas','relatorios','vipAlunos','aulasVip','boletins','prospectos']
    .some(k=>Array.isArray(s[k]) && s[k].length);
}
const MERGE_COLS=['usuarios','turmas','alunos','presencas','material','temas','testes','contatos','planos','comentarios','atas','tarefas','relatorios','vipAlunos','pacotesVip','aulasVip','boletins','eventos','comunicados','planejamento','recessos','alertas','apoios','contratos','writings','formacao','formacaoReg','formacaoSessoes','matriculas','financeiro','configFin','permissoes','estoqueMov','livroPedidos','prospectos'];
function marcarExcluido(col,id){
  if(id==null) return;
  S.exclusoes=S.exclusoes||[];
  S.exclusoes.push({col,id,ts:Date.now()});
  if(S.exclusoes.length>5000){ S.exclusoes.sort((a,b)=>(a.ts||0)-(b.ts||0)); S.exclusoes=S.exclusoes.slice(-5000); }
}
...
function mergeEstados(cloud, local){
  ...
  { const cL=(local.config&&local.config.atualizadoEm)||0, cC=(cloud.config&&cloud.config.atualizadoEm)||0;
    if(cL||cC) out.config=(cL>=cC?local.config:cloud.config); }   // config: o carimbo decide (antes, cópia velha revertia assinatura/e-mail)
  ...
  mt.sort((a,b)=>(a.ts||0)-(b.ts||0));
  out.exclusoes=mt.slice(-5000);
  return out;
}
async function cloudPut(force){
  ...
  marcarSync('...');
  // Compare-and-swap: só grava se a nuvem não mudou entre o pull e o envio.
  // Se outro aparelho gravou no meio, puxa de novo, re-mescla e tenta outra vez.
  for(let tent=0; tent<3; tent++){
    ...
    if(!force){
      try{
        ...
        if(cur.ok && cur.state && Array.isArray(cur.state.usuarios) && cur.state.usuarios.length){
        ...
        else if(cur.ok && !cur.state){ rev=cur.rev; /* nuvem REALMENTE vazia (primeira carga) → pode enviar o local */ }
        else {
          ...
          // acabou de salvar. Fica pendente e reenvia sozinho quando a conexão voltar.
    ...
    try{
      ...
      const { data, error }=await sb.from('estado').update({ dados:limpo, atualizado_em:Date.now() }).eq('id','app').eq('atualizado_em', rev).select('id');
      if(error){ marcarSync('off'); return {ok:false, adiado:true}; }
      if(data && data.length){ marcarSync('ok'); _marcarPendente(false); return {ok:true}; }
      // 0 linhas: alguém gravou no meio → repete o ciclo (pull+merge de novo)
    }catch(e){ marcarSync('off'); return {ok:false, adiado:true}; }
  }
  marcarSync('off'); return {ok:false, adiado:true};
}
...
function save(){
  ...
  catch(e){ try{ toast('⚠️ Memória do navegador cheia — os dados vão direto para a nuvem'); }catch(_){} }
...
async function puxarDaNuvem(){
  ...
  if(res.ok && res.state && res.state.usuarios && res.state.usuarios.length){
    const u=S.usuario, p=S.perfil;
    S=_pendenteSync ? mergeEstados(migrar(res.state), S) : migrar(res.state);   // não descarta edições ainda não enviadas
    S.usuario=u; S.perfil=p;
    localStorage.setItem(KEY,JSON.stringify(S));
    marcarSync(_pendenteSync?'...':'ok'); toast('Dados atualizados da nuvem ✓');
