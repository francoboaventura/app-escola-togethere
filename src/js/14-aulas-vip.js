...
function _cardPacoteVip(vid){
  ...
  if(!pac.length && !usado){ return _vipCardAberto(titulo, al, h+`<p class="hint" style="margin:8px 0 0">Nenhum pacote cadastrado.${ehGestor?' Clique em “+ Novo pacote” quando o aluno contratar horas.':''}</p>`); }
  ...
  return _vipCardAberto(titulo, al, h);
}
/* cartão sempre aberto (b175): título + selo de alerta + corpo, sem sanfona */
function _vipCardAberto(titulo, al, corpo, cor){
  // al: objeto de alerta do pacote OU texto pronto para o selo
  const selo=!al?'':(typeof al==='string'
    ?`<span class="pill" style="background:#fff3e0;color:#c2560b">⚠ ${al}</span>`
    :`<span class="pill" style="background:${(al.esgotado||al.vencido)?'#fdeaea':'#fff3e0'};color:${(al.esgotado||al.vencido)?'var(--vermelho)':'#c2560b'}" title="Renovar o pacote ou avisar o responsável">⚠ ${_vipAlertaTxt(al)}</span>`);
  return `<div class="card" style="border-left:4px solid ${cor||'var(--amarelo)'}">
    <div style="display:flex;align-items:center;gap:8px;flex-wrap:wrap"><h4 style="flex:1;margin:0">${titulo}</h4>${selo}</div>
    ${corpo}</div>`;
}
...
// Painel gestor (b180): filtros + cartões. A visão geral de horas é uma janela à parte (só direção).
function _vipPainelGestor(lista){
  ...
  const btnVG = (S.perfil==='direcao')?`<div style="margin:0 0 8px"><button class="btn ghost sm" onclick="abrirVisaoGeralVip()">📊 Visão geral — horas VIP${nAt?` <span class="pill" style="background:#fff3e0;color:#c2560b">${nAt} pedindo ação</span>`:''}</button></div>`:'';
  const conta=`<p class="hint" style="margin:10px 0 8px">${mostra.length} de ${ativos.length} aluno(s)</p>`;
  ...
  return _vipCardAberto('👑 Alunos VIP <span class="hint">'+ativos.length+' aluno(s)</span>', null, btnVG+filtros+conta+grade, '#C8A200');
}
// Janela dedicada (só direção): totais de horas + planilha + lista por aluno.
function setVgPer(p){ _vipPanoramaPer=p||'tudo'; abrirVisaoGeralVip(); }
function abrirVisaoGeralVip(){
  if(S.perfil!=='direcao') return toast('A visão geral de horas é exclusiva da direção');
  const lista=(S.vipAlunos||[]).filter(v=>!v.arquivado);
  ...
  const rows=com.map(v=>{ const c=vipContratadoMin(v.id),u=vipConsumoMin(v.id),s=c-u; const t=_vipSaldoTipo(v.id);
    return {v,c,u,s,urg:(t==='esgotado'?3:(t==='baixo'?2:0)),cor:_vipSaldoCor(t),uPer:temPer?vipConsumoMinPer(v.id,per):u}; }).sort((a,b)=>b.urg-a.urg||a.s-b.s);
  const nAt=rows.filter(r=>r.urg>0).length;
  const linha=r=>{ const pct=r.c>0?Math.min(100,Math.round(r.u/r.c*100)):(r.u>0?100:0);
    return `<div class="check" style="align-items:center"><span style="flex:1;min-width:0"><b>${esc(r.v.nome)}</b>${r.v.professor?` <span class="hint">· ${esc(r.v.professor)}</span>`:''}<br><span class="hint">${temPer?`<b style="color:#b8860b">${fmtDur(r.uPer)} no período</b> · `:''}${fmtDur(r.u)} de ${fmtDur(r.c)} (${pct}%) · saldo <b style="color:${r.cor}">${fmtDur(Math.max(0,r.s))}</b></span></span>
      <button class="btn ghost sm" onclick="fechar();abrirFicha('${r.v.id}')">abrir ›</button></div>`; };
  const opts=[['tudo','Desde o início'],['mes','Este mês'],['mespass','Mês passado'],['ano','Este ano']];
  modal(`<div style="display:flex;align-items:center;gap:8px;margin-bottom:6px"><h3 style="flex:1;margin:0">📊 Visão geral — horas VIP</h3><button class="close" onclick="fechar()">×</button></div>
    <div class="vip-toolbar" style="margin-bottom:8px"><select onchange="setVgPer(this.value)" style="max-width:150px">${opts.map(o=>`<option value="${o[0]}" ${per===o[0]?'selected':''}>${o[1]}</option>`).join('')}</select>
      <button class="btn ghost sm" onclick="exportarHorasVipCSV()">⬇️ Planilha</button></div>
    <div class="fx-tiles">${tile(com.length,'Alunos c/ pacote','#005EAF')}${tile(fmtDur(totC),'Contratadas','#005EAF')}${tile(fmtDur(temPer?totUPer:totU),temPer?'Utilizadas no período':'Utilizadas','#b8860b')}${tile(fmtDur(Math.max(0,totS)),'Saldo total',totS<=0?'var(--vermelho)':'var(--ok)')}</div>
    <p class="hint" style="margin:10px 0 6px">${nAt?`<b style="color:#c2560b">${nAt}</b> aluno(s) precisam de ação — aparecem primeiro.`:'Nenhum pacote pedindo ação agora. 👏'}</p>
    <div style="max-height:46vh;overflow:auto">${rows.length?rows.map(linha).join(''):'<p class="hint">—</p>'}</div>`);
}
...
VIEWS.vip=()=>{
  ...
  v.innerHTML=`<div class="section-title"><span class="feijao fj" style="background:#C8A200"></span><h2 class="display">👑 Alunos VIP</h2></div>
    <p class="sub">${ehSec?'Gestão dos alunos particulares — filtre e clique em “abrir aluno” para ver a ficha completa.':ehDir?'Aulas particulares — filtre e clique em “abrir aluno” para ver a ficha completa e lançar as aulas.':'Seus alunos particulares — clique num cartão para abrir a ficha e lançar as aulas.'}</p>
    ${ehGestor?_vipPainelGestor(lista):_vipCardsProfessor(lista)}
    ${cadastro}`;
...
function _ctAcaoVip(cor,titulo,prev,cta,fn){
  return `<div class="ctv" role="button" tabindex="0" onclick="${fn}" onkeydown="if(event.key==='Enter')${fn}">
    <div class="ct-topo" style="background:${cor}"></div>
    <div class="ct-miolo"><b>${titulo}</b><div class="hint" style="margin-top:4px">${prev}</div></div>
    <div class="ct-rodape"><span class="ct-abrir">${cta}</span></div></div>`;
}
function abrirVipLancar(){ const v=(S.vipAlunos||[]).find(a=>a.id===vipSel); if(!v)return; fichaLateral('➕ Lançar aula — '+esc(v.nome), ()=>{const x=(S.vipAlunos||[]).find(a=>a.id===vipSel); return x?_vipFormLancarHTML(x):'';}); }
function abrirVipAulas(){ const v=(S.vipAlunos||[]).find(a=>a.id===vipSel); if(!v)return; fichaLateral('👑 Aulas lançadas — '+esc(v.nome), ()=>vipSel?_vipAulasHTML(vipSel):''); }
function abrirVipWritings(){ const v=(S.vipAlunos||[]).find(a=>a.id===vipSel); if(!v)return; fichaLateral('✍️ Writings — '+esc(v.nome), ()=>vipSel?_vipWritingsHTML(vipSel):''); }
// escolher um aluno na lista: mostra os detalhes na própria tela
function selecionarVip(id){   // b180: abrir aluno = ficha completa (janela dedicada)
  if(typeof fecharFichaLateral==='function') fecharFichaLateral();
  if(id && typeof abrirFicha==='function') abrirFicha(id);
}
...
function addAulaVip(){
  ...
  if(typeof fecharFichaLateral==='function') fecharFichaLateral();   // b175: fecha a ficha de lançamento
...
// Quem pode mexer numa aula VIP já lançada (b168)
//  · direção: tudo · professor: as aulas de aula mesmo (não os cancelamentos da secretaria)
//  · secretaria: só os registros de cancelamento <12h, que é o que ela lança
function podeEditarAulaVip(a){
  if(!a || a.ajusteManual) return false;
  if(S.perfil==='direcao') return true;
  if(soLeitura()) return !!a.cancel12h;
  if(ehProfessor()) return !a.cancel12h && perm('prof_vip_lancar');
  return false;
}
function _vipReRender(){ if(typeof rota!=='undefined' && rota==='ficha' && VIEWS.ficha) VIEWS.ficha(); else if(VIEWS.vip) VIEWS.vip(); }
function delAulaVip(id){
  const a=(S.aulasVip||[]).find(x=>x.id===id);
  if(!podeEditarAulaVip(a)) return toast(soLeitura()?'A secretaria só edita os cancelamentos que registra':'Sem permissão para excluir esta aula');
  if(!confirm(a.cancel12h?'Excluir este registro de cancelamento? A hora volta para o saldo do aluno.':'Excluir esta aula VIP? A hora volta para o saldo do aluno.'))return;
  S.aulasVip=S.aulasVip.filter(x=>x.id!==id); marcarExcluido('aulasVip',id); save(); _vipReRender(); toast('Registro excluído');
}
function editarAulaVip(id){
  const a=S.aulasVip.find(x=>x.id===id);
  if(!podeEditarAulaVip(a))return toast(soLeitura()?'A secretaria só edita os cancelamentos que registra':'Sem permissão para editar esta aula');
  const ehCancel=!!a.cancel12h;
  modal(`<h3>${ehCancel?'✏️ Editar cancelamento':'✏️ Editar aula VIP'} <button class="close" onclick="fechar()">×</button></h3>
    ${ehCancel?'<p class="hint" style="margin:0 0 10px;color:#c2560b">🚫 Registro de <b>cancelamento em cima da hora</b> (menos de 12h): a aula segue como não realizada e a hora continua descontada do pacote. Para desfazer, use “🗑 excluir”.</p>':''}
    ...
    ${ehCancel?'':`<label style="display:flex;align-items:center;gap:8px;cursor:pointer;font-size:.9rem;margin:2px 0 6px"><input type="checkbox" id="evFaltou" ${a.faltou?'checked':''} onchange="document.getElementById('evCobrarWrap').style.display=this.checked?'block':'none'"> ❌ Aluno não compareceu</label>
    <div id="evCobrarWrap" style="display:${a.faltou?'block':'none'};margin:0 0 10px 22px">${ehProfessor()
      ?'<p class="hint" style="margin:0;color:#b8860b">💳 A hora é descontada do pacote (regra da escola).</p>'
      :`<label style="display:flex;align-items:center;gap:8px;cursor:pointer;font-size:.9rem;color:#b8860b"><input type="checkbox" id="evCobrarFalta" ${a.cobrarFalta?'checked':''}> 💳 Descontar a hora do pacote</label>`}</div>`}
    <div style="display:flex;gap:8px;flex-wrap:wrap;margin-top:4px">
      <button class="btn" onclick="salvarEdicaoAulaVip('${id}')">💾 Salvar alterações</button>
      <button class="btn ghost" style="color:var(--vermelho)" onclick="delAulaVip('${id}')">🗑 Excluir</button>
...
function salvarEdicaoAulaVip(id){
  ...
  a.duracaoMin=_dv;
  if(a.cancel12h){ a.faltou=true; a.cobrarFalta=true; }        // cancelamento <12h mantém a regra
  else {
    a.faltou=!!(document.getElementById('evFaltou')||{}).checked;
    a.cobrarFalta=a.faltou && (ehProfessor() ? true : !!((document.getElementById('evCobrarFalta')||{}).checked));
  }
  a.atualizadoEm=Date.now();                 // carimbo para a sincronização entre aparelhos
  save(); fechar(); _vipReRender(); toast(a.cancel12h?'Cancelamento atualizado':'Aula atualizada');
}
