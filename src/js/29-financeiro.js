...
function _finDatas(f, n){   // n datas de vencimento conforme 1º venc / dia / início
  n=Math.max(0,parseInt(n)||0); if(!n) return [];
  const dia=Math.min(28,Math.max(1,parseInt(f.diaVencimento)||10)); const out=[];
  if(f.primeiroVenc && /^\d{4}-\d{2}-\d{2}$/.test(f.primeiroVenc)){
    const p1=new Date(f.primeiroVenc+'T12:00:00'); out.push(ymd(p1));
    for(let i=1;i<n;i++) out.push(ymd(new Date(p1.getFullYear(), p1.getMonth()+i, dia)));
  } else {
    const base=new Date(finInicio(f)+'T12:00:00'); const shift=(dia<base.getDate())?1:0;
    for(let i=0;i<n;i++) out.push(ymd(new Date(base.getFullYear(), base.getMonth()+i+shift, dia)));
  }
  return out;
}
function finParcelas(f){ const n=Math.max(0,parseInt(f.parcelas)||0); if(!n) return []; const val=finMensalLiquida(f); return _finDatas(f,n).map((venc,i)=>({n:i+1, venc, valor:val})); }
// b182: material é uma cobrança à parte, parcelável, alinhada às mesmas datas das mensalidades
function finMaterialTotal(f){ return _matN(f&&f.materialTotal); }
function finParcelasMaterial(f){
  const tot=finMaterialTotal(f); const n=Math.max(0,parseInt(f&&f.materialParcelas)||0); if(!tot||!n) return [];
  const base=Math.floor(tot/n*100)/100;
  return _finDatas(f,n).map((venc,i)=>({n:i+1, venc, valor:(i===n-1)?(Math.round((tot-base*(n-1))*100)/100):base}));
}
function finMatPago(f,pn){ return !!((f&&f.pagosMaterial)||{})[pn]; }
function finMaterialRecebido(f){ const pg=(f&&f.pagosMaterial)||{}; return finParcelasMaterial(f).reduce((s,p)=>s+(pg[p.n]?(pg[p.n].valor!=null?_matN(pg[p.n].valor):p.valor):0),0); }
function finMaterialAberto(f){ const pg=(f&&f.pagosMaterial)||{}; return finParcelasMaterial(f).reduce((s,p)=>s+(pg[p.n]?0:p.valor),0); }
function finMaterialAtrasado(f){ const pg=(f&&f.pagosMaterial)||{}; return finParcelasMaterial(f).reduce((s,p)=>s+((!pg[p.n]&&p.venc<hoje())?p.valor:0),0); }
// cronograma combinado; com f.somarMesmoDia junta o que cai no mesmo vencimento
function finCronograma(f){
  const mens=finParcelas(f).map(p=>({tipo:'mensalidade',n:p.n,venc:p.venc,valor:p.valor,pago:!!(f.pagos||{})[p.n]}));
  const mat=finParcelasMaterial(f).map(p=>({tipo:'material',n:p.n,venc:p.venc,valor:p.valor,pago:finMatPago(f,p.n)}));
  const todos=mens.concat(mat);
  if(!f.somarMesmoDia) return todos.sort((a,b)=>a.venc.localeCompare(b.venc)||(a.tipo<b.tipo?-1:1)).map(it=>({venc:it.venc, itens:[it], total:it.valor, paga:it.pago}));
  const by={}; todos.forEach(it=>{ (by[it.venc]=by[it.venc]||[]).push(it); });
  return Object.keys(by).sort().map(venc=>({venc, itens:by[venc], total:by[venc].reduce((s,i)=>s+i.valor,0), paga:by[venc].every(i=>i.pago)}));
}
function finTogglePag(id, tipo, n){
  if(S.perfil!=='direcao') return toast('Sem permissão');
  const f=(S.financeiro||[]).find(x=>x.id===id); if(!f) return;
  if(tipo==='material'){ f.pagosMaterial=f.pagosMaterial||{};
    if(f.pagosMaterial[n]) delete f.pagosMaterial[n];
    else { const p=finParcelasMaterial(f).find(x=>x.n===n); f.pagosMaterial[n]={em:hoje(), valor:p?p.valor:0, forma:f.formaPagamento||''}; }
  } else { f.pagos=f.pagos||{};
    if(f.pagos[n]) delete f.pagos[n];
    else { const p=finParcelas(f).find(x=>x.n===n); const at=(p&&finParcelaStatus(f,p)==='atrasada')?finValorAtualizado(p):null; f.pagos[n]={em:hoje(), valor:at?at.total:(p?p.valor:0), forma:f.formaPagamento||''}; }
  }
  f.atualizadoEm=Date.now(); save(); abrirCarne(id);
}
function finToggleLinha(id, venc){
  if(S.perfil!=='direcao') return toast('Sem permissão');
  const f=(S.financeiro||[]).find(x=>x.id===id); if(!f) return;
  f.pagos=f.pagos||{}; f.pagosMaterial=f.pagosMaterial||{};
  const mens=finParcelas(f).filter(p=>p.venc===venc), mat=finParcelasMaterial(f).filter(p=>p.venc===venc);
  const aberto = mens.some(p=>!f.pagos[p.n]) || mat.some(p=>!f.pagosMaterial[p.n]);
  if(aberto){
    mens.forEach(p=>{ if(!f.pagos[p.n]){ const at=(finParcelaStatus(f,p)==='atrasada')?finValorAtualizado(p):null; f.pagos[p.n]={em:hoje(), valor:at?at.total:p.valor, forma:f.formaPagamento||''}; } });
    mat.forEach(p=>{ if(!f.pagosMaterial[p.n]) f.pagosMaterial[p.n]={em:hoje(), valor:p.valor, forma:f.formaPagamento||''}; });
  } else { mens.forEach(p=>delete f.pagos[p.n]); mat.forEach(p=>delete f.pagosMaterial[p.n]); }
  f.atualizadoEm=Date.now(); save(); abrirCarne(id);
}
...
function finTotalRecebido(){ return (S.financeiro||[]).reduce((s,f)=>s+finRecebido(f)+finMaterialRecebido(f)+finExtrasRecebido(f),0); }   // dinheiro recebido não some ao cancelar/editar
function finTotalAberto(){ return _finScope().reduce((s,f)=>s+finAberto(f)+finMaterialAberto(f)+finExtrasAberto(f),0); }
function finTotalAtrasado(){ return _finScope().filter(f=>finStatusMat(f)!=='trancada').reduce((s,f)=>s+finAtrasadoV(f)+finMaterialAtrasado(f),0); }
function finInadimplentes(){ return _finScope().filter(f=>finStatusMat(f)!=='trancada' && (finAtrasadoV(f)+finMaterialAtrasado(f))>0).length; }
...
function abrirFinanceiro(id){
  ...
  modal(`<div style="display:flex;align-items:center;gap:8px;margin-bottom:4px"><h3 style="flex:1;margin:0">💰 Financeiro — ${esc(finNome(f))}</h3><button class="close" onclick="fechar()">×</button></div>
    ...
    <div class="row"><div class="field"><label class="lbl">📚 Material (R$) <span class="hint">cobrança à parte</span></label><input type="number" id="fin_materialTotal" value="${f.materialTotal!=null&&_matN(f.materialTotal)>0?f.materialTotal:''}" min="0" step="0.01" placeholder="0,00" oninput="_finResumo()"></div>
      <div class="field"><label class="lbl">Material: parcelas</label><input type="number" id="fin_materialParcelas" value="${f.materialParcelas!=null?f.materialParcelas:1}" min="1" max="48" oninput="_finResumo()"></div></div>
    <label style="display:flex;align-items:center;gap:8px;cursor:pointer;font-size:.9rem;margin:0 0 10px"><input type="checkbox" id="fin_somarMesmoDia" ${f.somarMesmoDia?'checked':''}> ➕ Somar as parcelas que caem no mesmo dia <span class="hint">(mensalidade + material num só boleto)</span></label>
...
function abrirCarne(id){
  if(S.perfil!=='direcao') return toast('Sem permissão');
  const f=(S.financeiro||[]).find(x=>x.id===id); if(!f) return;
  const cron=finCronograma(f);
  if(!cron.length) return toast('Defina mensalidade e parcelas primeiro');
  const trancada=finStatusMat(f)==='trancada';
  const chipSt={paga:{t:'paga',bg:'#eafaf0',c:'#0A7A3D'},atrasada:{t:'em atraso',bg:'#fdeaea',c:'#E52524'},aberta:{t:'a vencer',bg:'#eef2f8',c:'#5a6b86'}};
  const rotulo=it=>it.tipo==='material'?('📚 material '+it.n):('mensalidade '+it.n);
  const linhas=cron.map(l=>{ const st=l.paga?'paga':((!trancada && l.venc<hoje())?'atrasada':'aberta'); const cs=chipSt[st];
    const combinado=l.itens.length>1;
    const desc=combinado?(' <span class="hint">('+l.itens.map(i=>`${i.tipo==='material'?'material':'mensalidade'} ${_moeda(i.valor)}`).join(' + ')+')</span>'):'';
    const toggleFn=combinado?`finToggleLinha('${id}','${l.venc}')`:`finTogglePag('${id}','${l.itens[0].tipo}',${l.itens[0].n})`;
    return `<div class="check" style="display:block"><div style="display:flex;gap:8px;align-items:center;flex-wrap:wrap">
      <b style="min-width:74px">${combinado?('dia '+brDate(l.venc)):rotulo(l.itens[0])}</b>
      <span style="flex:1">venc. ${brDate(l.venc)} · <b>${_moeda(l.total)}</b>${desc}</span>
      <span class="pill" style="background:${cs.bg};color:${cs.c}">${cs.t}</span>
      ${!l.paga?`<button class="btn ghost sm" title="Boleto (simulação)" onclick="gerarBoletoLinha('${id}','${l.venc}')">🧾</button>`:''}
      <button class="btn ghost sm" onclick="${toggleFn}">${l.paga?'desfazer':'✓ receber'}</button>
    </div></div>`;
  }).join('');
  const rec=finRecebido(f)+finMaterialRecebido(f), abe=finAberto(f)+finMaterialAberto(f), atr=finAtrasadoV(f)+finMaterialAtrasado(f);
  modal(`<div style="display:flex;align-items:center;gap:8px;margin-bottom:4px"><h3 style="flex:1;margin:0">💳 Carnê — ${esc(finNome(f))}</h3><button class="close" onclick="fechar()">×</button></div>
    <div class="fx-tiles" style="margin:2px 0 12px">
      <div class="fx-tile"><div class="v" style="color:#0A7A3D">${_moeda(rec)}</div><div class="l">Recebido</div></div>
      <div class="fx-tile"><div class="v" style="color:#005EAF">${_moeda(abe)}</div><div class="l">A receber</div></div>
      <div class="fx-tile"><div class="v" style="color:${atr>0?'var(--vermelho)':'var(--tinta)'}">${_moeda(atr)}</div><div class="l">Em atraso</div></div>
    </div>${finMaterialTotal(f)>0?`<p class="hint" style="margin:0 0 8px">📚 Material: <b>${_moeda(finMaterialTotal(f))}</b> em ${finParcelasMaterial(f).length}× ${f.somarMesmoDia?'· <b>somado</b> às mensalidades do mesmo dia':'· cobrança à parte'}.</p>`:''}${linhas}
...
function _boletoDoc(f, venc, valor, linha2, extra){
  const linhaDigitavel='00000.00000 00000.000000 00000.000000 0 '+String(venc.replace(/-/g,''))+String(Math.round(valor*100)).padStart(10,'0');
...
<div class="bol">
  <div class="l"><span><b>Beneficiário:</b> Togethere Escola de Idiomas</span><span><b>Vencimento:</b> ${brDate(venc)}</span></div>
  <div class="l"><span><b>Pagador:</b> ${esc(finNome(f))}</span><span>${linha2||''}</span></div>
  <div class="l"><span><b>Valor do documento:</b> ${_moeda(valor)}</span><span>${extra||''}</span></div>
...
function gerarBoletoSim(id,n){
  ...
  const at=finParcelaStatus(f,p)==='atrasada'?finValorAtualizado(p):null; const valor=at?at.total:p.valor;
  _boletoDoc(f, p.venc, valor, '<b>Parcela:</b> '+p.n+'/'+f.parcelas, at&&at.total>p.valor?('<b>c/ multa e juros:</b> '+_moeda(at.total)+' ('+at.dias+'d)'):'');
}
// b182: boleto de um dia — soma mensalidade + material que vencem na mesma data
function gerarBoletoLinha(id, venc){
  const f=(S.financeiro||[]).find(x=>x.id===id); if(!f) return;
  let total=0; const partes=[];
  finParcelas(f).filter(p=>p.venc===venc).forEach(p=>{ const at=finParcelaStatus(f,p)==='atrasada'?finValorAtualizado(p):null; total+=(at?at.total:p.valor); partes.push('mensalidade '+p.n); });
  finParcelasMaterial(f).filter(p=>p.venc===venc).forEach(p=>{ total+=p.valor; partes.push('material '+p.n); });
  if(!total) return;
  _boletoDoc(f, venc, total, partes.join(' + '), '');
}
